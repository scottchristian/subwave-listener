    ...
    <main id="main-player" className="container" style={{ paddingTop: "2rem", overflowX: "clip" }}>
      ...
      <header id="header-player" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "3rem", flexWrap: "wrap", gap: "1rem" }}>
        ...
        <div id="header-actions" style={{ display: "flex", gap: "1rem", alignItems: "center" }}>
          <button id="btn-liked-songs" ref={el => { stepRefs.current[7] = el; }} onClick={() => setShowLikes(true)} className="primary-btn" style={{ padding: "0.5rem 1rem", fontSize: "0.875rem", background: "var(--color-surface)", border: "1px solid var(--color-border)", color: "var(--color-text)", width: "auto", position: tourStep === 7 ? "relative" : "static", zIndex: tourStep === 7 ? 1000 : 1 }}>Liked Songs</button>
          {(session.user as any)?.isAdmin && (
            <a id="btn-admin" href="/admin" className="primary-btn" style={{ padding: "0.5rem 1rem", fontSize: "0.875rem", background: "var(--color-surface)", border: "1px solid var(--color-border)", color: "var(--color-text)", width: "auto" }}>Admin</a>
          )}
          <button id="btn-signout" className="primary-btn" onClick={() => signOut({ callbackUrl: "/" })} style={{ padding: "0.5rem 1rem", fontSize: "0.875rem", background: "var(--color-surface)", border: "1px solid var(--color-border)", color: "var(--color-text)", width: "auto" }}>Sign out</button>
        </div>
      </header>
